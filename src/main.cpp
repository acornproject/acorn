...
const string strMessageMagic = "Acorn Signed Message:\n";
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfWork())
    {
	    ...
	    CBitcoinAddress address(!fTestNet ? ENDORSEMENTADDRESS : ENDORSEMENTADDRESSTESTNET);
        CScript scriptPubKey;
        scriptPubKey.SetDestination(address.Get());
        if (vtx[0].vout[1].scriptPubKey != scriptPubKey)
            return error("ConnectBlock() : no endorsement)");
        if (vtx[0].vout[1].nValue < endorsement)
            return error("ConnectBlock() : endorsement value too small");
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Acorn", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "China reverts to credit as property slump threatens to drag down economy.";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("0x5e826613b9705b18788176c5d4fa6cabb95e36b936fcfee08378e6945c322370"));
